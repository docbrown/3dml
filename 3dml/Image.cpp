...
image_cleanup(void)
{
	for (int index = 0; index < pixmaps; index++)
		if (pixmap_list[index].image_ptr) {
			DELARRAY(pixmap_list[index].image_ptr, imagebyte, 
...
image_error(const char *format, ...)
{
	...
	vsprintf(error_msg, format, arg_ptr);
	va_end(arg_ptr);
...
image_memory_error(const char *object)
{
	...
	sprintf(error_msg, "Insufficient memory to allocate %s", object);
	throw (char *)error_msg;
}
...
load_JPEG(void)
{
	...
	// Initialise the totaltime for animation.
		   ...
		   bool unlimited_size)
{
	...
	// assumed this is a style texture found in the style ZIP archive.

	if (URL) {
		if (!push_file(file_path, false)) {
