							...
							COL_AABOX *aaBox_p, float maxStepHeight)
{
	...
	downBox.maxDim.y = aaBox_p->maxDim.y + world_ptr->half_block_units;
	downBox.maxDim.z = aaBox_p->maxDim.z;

	downBox.offsToCentre.x = aaBox_p->offsToCentre.x;
	downBox.offsToCentre.y = -(world_ptr->block_units - downBox.maxDim.y); 
	...
	nearestY = y - world_ptr->block_units;
				...
				if (colliding)
				{
					...
					if (mod > 0)
					{
							...
							oldX, oldY, oldZ, aaBox_p, &centreOfBox) )
						{
							...
							//------ Shove the position back to ---- 
							//------ the old one, and leave it at --
							//------ that! -------------------------
